...
from src.infrastructure.services.auth.base import AbstractAuthService, AuthServiceException
from src.web.schemas import UserOut

from . import ContainerDep


async def get_current_user_from_auth_header(
...
async def get_current_user_from_cookie_token(
    container: ContainerDep,
    ...
    if not token:
        raise HTTPException(status.HTTP_401_UNAUTHORIZED)
    service: AbstractAuthService = container.resolve(AbstractAuthService)  # type: ignore
